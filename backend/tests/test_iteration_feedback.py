...
from app.services.agent_service import get_strict_plan_schema
...
def build_model_payload_response(user_content: str) -> str:
    payload_str = user_content.split("Create a safe draft plan using this context:\n", 1)[1]
    ...
    exercise_pool = payload["exercise_pool"]
    sessions_per_week = blueprint["sessions_per_week"]
    duration = blueprint["session_duration_min"]
    max_rpe = constraints["hard"]["max_intensity_rpe"]
    rest_days = constraints["hard"]["min_rest_days_per_week"]

    valid_days = ["Lunes", "Miércoles", "Viernes", "Sábado", "Domingo"]
    mobility = next(ex for ex in exercise_pool if ex["modality"] == "mobility")
    main_exercise = next(ex for ex in exercise_pool if ex["id"] != mobility["id"])

    sessions = []
    weekly_schedule = []
    for index in range(sessions_per_week):
        session_code = chr(65 + index)
        day_label = valid_days[index]
        sessions.append(
            {
                ...
                "main": [
                    {
                        ...
                        "name": main_exercise["name"],
                        "minutes": max(10, duration - 10),
                        "rpe": min(max_rpe, 5),
                        "tags": [],
                    }
                ],
        ...
        weekly_schedule.append(f"{day_label}:{session_code}")
    ...
    return json.dumps(
        {
            ...
            "sessions": sessions,
            "progression_notes": "Mantener o progresar suavemente.",
            "rationale_summary": "Plan generado por test end-to-end.",
            "questions_to_finalize": [],
            "assumptions": [],
            "safety_flags": [],
        }
...
class FakeResponses:
    def create(self, *, model, input, text):
        ...
        return type("Resp", (), {"output_text": build_model_payload_response(user_content)})()
...
class IterationFeedbackTests(unittest.TestCase):
    ...
    def test_strict_plan_schema_sets_additional_properties_false(self):
        schema = get_strict_plan_schema()
        self.assertFalse(schema.get("additionalProperties", True))

        defs = schema.get("$defs", {})
        object_defs = [
            value for value in defs.values()
            if isinstance(value, dict) and value.get("type") == "object"
        ]
        self.assertTrue(object_defs)
        self.assertTrue(all(d.get("additionalProperties") is False for d in object_defs))
    ...
    @patch("app.services.agent_service.get_client", return_value=FakeOpenAIClient())
    def test_full_flow_with_mocked_openai(self, _mock_client):
        client = TestClient(app)

        intake_response = client.post(
            "/intake/",
            params={"user_id": "user-e2e"},
            json={
                "age": 35,
                "sex": "female",
                "weight_kg": 65,
                ...
                "activity_history_text": "Retomando actividad",
                "injuries_text": "",
                "medical_flags": [],
                "time_per_session_min": 30,
                "days_per_week_preferred": 3,
        ...
        draft_response = client.post("/plan/draft", params={"user_id": "user-e2e"})
        self.assertEqual(draft_response.status_code, 200)
        plan_id = draft_response.json()["plan"]["plan_id"]
        self.assertEqual(draft_response.json()["plan"]["sessions_per_week"], 3)

        iterate_response = client.post(
            f"/plan/{plan_id}/iterate",
            params={"user_id": "user-e2e"},
            json={"user_message": "Quiero 2 días y sesiones de 25 minutos"},
        )
        self.assertEqual(iterate_response.status_code, 200)
        self.assertEqual(iterate_response.json()["plan"]["sessions_per_week"], 2)
        self.assertEqual(iterate_response.json()["plan"]["sessions"][0]["estimated_minutes"], 25)

        finalize_response = client.post(
            f"/plan/{plan_id}/finalize",
            params={"user_id": "user-e2e"},
            json={"confirm": True},
        )
        self.assertEqual(finalize_response.status_code, 200)
